...
public class AnalyseStockToolImpl implements AnalyseStockTool {
	...
	public void analyseAccumuChangePct(AnalyseCondition condition) {
		...
		if (accumuDays != null && accumuDaysChangePctLower != null
				&& accumuDaysChangePctUpper != null) {
		...
		// 查询数据库.
		List<StockCapFlow> analyseList = stockCapFlowService
				.analyseFall(condition);
		 ...
		 * 公共过滤
		 */
		List<StockCapFlow> filteredAnalyseList = commonFilter(analyseList,
				condition, codeNameMap, codeIndustryNameMap);
		...
		for (StockCapFlow stock : filteredAnalyseList) {
			...
			if (accumuMap != null && !accumuMap.isEmpty()) {
				...
				if (accumuChangePct == null
						|| accumuChangePct < accumuDaysChangePctLower
						|| accumuChangePct > accumuDaysChangePctUpper) {
					continue;
				}
			}
			...
			formatter.format(
					"%7s %-10s %-10s %-6s %-16s %-16s %-16s %-16s %-16s\n", code,
					name, codeIndustryNameMap.get(code), stock.getChangePercent(),
					"主力: " + stock.getMainNetIn() + "万",
					"超大单: " + stock.getSuperLarge() + "万", "大单: " + stock.getLarge()
							+ "万", "中单: " + stock.getMiddle() + "万",
					"小单: " + stock.getSmall() + "万");
	...
	public void analyseAccumuMainNetIn(AnalyseCondition condition) {
		...
		Map<String, Float> accumuMap = new HashMap<String, Float>();
		if (accumuDays != null && accumuMainNetInPctLower != null
				&& accumuMainNetInPctUpper != null) {
			accumuMap = getAccumuMap(accumuDays, condition.getTradeDate());
			if (accumuMap == null) {
				return;
			}
		...
		for (StockCapFlow stock : filteredAnalyseList) {
			...
			if (accumuMap != null && !accumuMap.isEmpty()) {
				if (accumuMainNetInPct == null
						|| accumuMainNetInPct < accumuMainNetInPctLower
						|| accumuMainNetInPct > accumuMainNetInPctUpper) {
					continue;
				}
			}
			...
			Formatter formatter = new Formatter();
			formatter.format(
					"%7s %-10s %-10s %-6s %-6s %-16s %-16s %-16s %-16s\n", code,
					name, codeIndustryNameMap.get(code), stock.getClose(),
					stock.getChangePercent(), "累计主力净占比: " + accumuMainNetInPct,
					"主力: " + stock.getMainNetIn() + "万",
					"超大单: " + stock.getSuperLarge() + "万", "大单: " + stock.getLarge()
							+ "万");
			sb.append(formatter.toString());
	...
	private boolean init(AnalyseCondition condition) {
		 ...
		 */
		codeIndustryNameMap = industryStockService
				.queryCodeIndustryNameMap(StockConstant.THS_FLAG);
	...
	private void fillBlankCondition(AnalyseCondition originalCondition) {
		if (originalCondition.getTradeDate() == null) {
			String tradeDateStr = new SimpleDateFormat("YYYY-MM-dd")
					.format(Calendar.getInstance().getTime()) + " 15:00:00";
	...
	private boolean isValid(AnalyseCondition condition) {
		if (!isTradeDateValid(condition.getTradeDate())
				|| !isAccumuDaysValid(condition.getAccumuDays())) {
			return false;
		}
		return true;
	}
	...
	private boolean isTradeDateValid(Timestamp tradeDate) {
		...
		List<StockCapFlow> list = stockCapFlowService
				.queryCapFlowByDate(tradeDate);
		if (list == null || list.isEmpty()) {
			LOGGER.info("tradeDate: " + tradeDate
					+ " not exists in ST_STOCK_CAP_FLOW, return now...");
			return false;
		}
		return true;
	}
	...
	private boolean isAccumuDaysValid(Integer accumuDays) {
		if (accumuDays != null && accumuDays.intValue() > 10) {
			LOGGER.info("accumuDays: " + accumuDays
					+ " must less than 10, return now...");
			return false;
		}
		return true;
	}
	 ...
	 * @return
	 */
	private Map<String, Float> getAccumuMap(Integer accumuDays,
			Timestamp tradeDate) {
		List<Timestamp> dates = stockCapFlowService.queryTradeDateBefore(
				tradeDate, "000001");
		...
		List<StockCapFlow> capFlowList = stockCapFlowService
				.queryAllAccumuChangePct(tradeDateLower, tradeDate);
		if (capFlowList == null) {
	 ...
	 * @return
	 */
	private List<StockCapFlow> commonFilter(List<StockCapFlow> analyseList,
			AnalyseCondition condition, Map<String, String> codeNameMap,
		...
		for (StockCapFlow capFlow : analyseList) {
			 ...
			 */
			if (types != null
					&& !types.contains(Integer.valueOf(code.substring(0, 1)))) {
				continue;
			}
			 ...
			 * 收盘价过滤
			 */
			if (close == null || close < condition.getCloseLower()
					|| close > condition.getCloseUpper()) {
				continue;
			}
	...
	public void genNotionHotCsv(int year) {
		...
		List<NotionHot> notionHotList = notionHotService
				.queryNotionHotForCsv(year);
		if (notionHotList == null) {
			return;
		}
		StringBuilder sb = new StringBuilder(
				"日期,上证指数(%),上证成交量(亿),深证指数(%),深证成交量(亿),创业板指数(%),创业板成交量(亿),序号,概念名称,涨跌幅,领涨股,涨跌幅,时间\n");
		SimpleDateFormat format1 = new SimpleDateFormat("YYYY-MM-dd");
		SimpleDateFormat format2 = new SimpleDateFormat("YYYY-MM-dd HH:mm:ss");
		Map<String, Float> indexMap = indexService
				.queryAllChangePctAndVolumnAmnt();
		if (indexMap == null) {
			return;
		}
		for (NotionHot notionHot : notionHotList) {
			...
			String changePctSH = indexMap.containsKey(changePctSHKey) ? String
					.valueOf(indexMap.get(changePctSHKey)) : "";
			String changePctSZ = indexMap.containsKey(changePctSZKey) ? String
					.valueOf(indexMap.get(changePctSZKey)) : "";
			String changePctCZ = indexMap.containsKey(changePctCZKey) ? String
					.valueOf(indexMap.get(changePctCZKey)) : "";
			String volumnAmntSH = indexMap.containsKey(volumnAmntSHKey) ? String
					.valueOf(indexMap.get(volumnAmntSHKey)) : "";
			String volumnAmntSZ = indexMap.containsKey(volumnAmntSZKey) ? String
					.valueOf(indexMap.get(volumnAmntSZKey)) : "";
			String volumnAmntCZ = indexMap.containsKey(volumnAmntCZKey) ? String
					.valueOf(indexMap.get(volumnAmntCZKey)) : "";

			sb.append(tradeDateStr).append(CSV_SPLIT).append(changePctSH)
					.append(CSV_SPLIT).append(volumnAmntSH).append(CSV_SPLIT)
					.append(changePctSZ).append(CSV_SPLIT).append(volumnAmntSZ)
					.append(CSV_SPLIT).append(changePctCZ).append(CSV_SPLIT)
					...
					.append(notionHot.getRiseLeadStockChangePercent())
					.append(CSV_SPLIT)
					.append(format2.format(notionHot.getTimestamp())).append("\n");
		}
		StockUtils.writeToFile(
				"/home/leslie/MyProject/StockAnalyse/gen/notionHot" + "_" + year
						+ ".csv", sb, "GB2312");
	...
	public void genIndustryHotCsv(int year) {
		long begin = System.currentTimeMillis();
		List<IndustryHot> industryHotList = industryHotService
				.queryIndustryHotForCsv(year);
		if (industryHotList == null) {
			return;
		}
		StringBuilder sb = new StringBuilder(
				"日期,上证指数(%),上证成交量(亿),深证指数(%),深证成交量(亿),创业板指数(%),创业板成交量(亿),序号,行业名称,涨跌幅,领涨股,涨跌幅,时间\n");
		SimpleDateFormat format1 = new SimpleDateFormat("YYYY-MM-dd");
		SimpleDateFormat format2 = new SimpleDateFormat("YYYY-MM-dd HH:mm:ss");
		Map<String, Float> indexMap = indexService
				.queryAllChangePctAndVolumnAmnt();
		...
		for (IndustryHot industryHot : industryHotList) {
			...
			sb.append(tradeDateStr).append(CSV_SPLIT).append(changePctSH)
					...
					.append(industryHot.getRiseLeadStockChangePercent())
					.append(CSV_SPLIT)
					.append(format2.format(industryHot.getTimestamp())).append("\n");
		}
		StockUtils.writeToFile(
				"/home/leslie/MyProject/StockAnalyse/gen/industryHot" + "_" + year
						+ ".csv", sb, "GB2312");
		long end = System.currentTimeMillis();
		LOGGER.info("elapsed time: " + (end - begin) / (1000) + " s.");
	}
	...
	public void genIndustryHotStockCsv(int year) {
		long begin = System.currentTimeMillis();
		List<IndustryHotStocks> list = industryHotStocksService
				.queryIndustryHotStocksForCsv(year);
		if (list == null) {
			return;
		}
		LOGGER.info("queryIndustryHotStocksForCsv elapsed time: "
				+ (System.currentTimeMillis() - begin) / 1000 + " s.");

		Map<String, Integer> nameNumMap = industryInfoService
				.queryIndustryCorpsNumMap(StockConstant.THS_FLAG);
		if (nameNumMap == null) {
			return;
		}

		StringBuilder sb = new StringBuilder(
				"日期,序号,行业名称,涨跌幅,序号,股票代码,股票名称,最新价,涨跌幅,时间\n");
		SimpleDateFormat format1 = new SimpleDateFormat("YYYY-MM-dd");
		SimpleDateFormat format2 = new SimpleDateFormat("YYYY-MM-dd HH:mm:ss");
		// 显示多少个股票
		int rankLimit = 30;
		for (IndustryHotStocks stock : list) {
			...
			sb.append(format1.format(stock.getTradeDate())).append(CSV_SPLIT)
					.append(industryRank).append(CSV_SPLIT).append(industryName)
					.append(CSV_SPLIT).append(stock.getIndustryChangePercent())
					.append(CSV_SPLIT).append(rank).append(CSV_SPLIT)
		...
		StockUtils.writeToFile(
				"/home/leslie/MyProject/StockAnalyse/gen/industryHotStocks" + "_"
						+ year + ".csv", sb, "GB2312");
		LOGGER.info("writeToFile elapsed time: "
				+ (System.currentTimeMillis() - beforeWriteTime) / 1000 + " s.");
	...
	public void genNotionHotStockCsv(int year) {
		long begin = System.currentTimeMillis();
		List<NotionHotStocks> list = notionHotStocksService
				.queryNotionHotStocksForCsv(year);
		if (list == null) {
			return;
		}
		LOGGER.info("queryNotionHotStocksForCsv elapsed time: "
				+ (System.currentTimeMillis() - begin) / 1000 + " s.");

		Map<String, Integer> nameNumMap = notionInfoService
				.queryNotionNameNumMap(StockConstant.THS_FLAG);
		if (nameNumMap == null) {
			return;
		}

		StringBuilder sb = new StringBuilder(
				"日期,序号,概念名称,涨跌幅,序号,股票代码,股票名称,最新价,涨跌幅,时间\n");
		SimpleDateFormat format1 = new SimpleDateFormat("YYYY-MM-dd");
		SimpleDateFormat format2 = new SimpleDateFormat("YYYY-MM-dd HH:mm:ss");
		// 显示多少个股票
		int rankLimit = 30;
		for (NotionHotStocks stock : list) {
			...
			sb.append(format1.format(stock.getTradeDate())).append(CSV_SPLIT)
					.append(notionRank).append(CSV_SPLIT).append(notionName)
					.append(CSV_SPLIT).append(stock.getNotionChangePercent())
					.append(CSV_SPLIT).append(rank).append(CSV_SPLIT)
					.append(stock.getCode()).append(CSV_SPLIT)
					.append(stock.getStockName()).append(CSV_SPLIT)
					.append(stock.getNewPrice()).append(CSV_SPLIT)
		...
		long beforeWriteTime = System.currentTimeMillis();
		StockUtils.writeToFile(
				"/home/leslie/MyProject/StockAnalyse/gen/notionHotStocks" + "_"
						+ year + ".csv", sb, "GB2312");
		LOGGER.info("writeToFile elapsed time: "
				+ (System.currentTimeMillis() - beforeWriteTime) / 1000 + " s.");
	...
	public StringBuilder getNotionHotPhraseString(String lowTradeDateStr,
			String highTradeDateStr, boolean header) {
		long begin = System.currentTimeMillis();
		// lowTradeDateStr <= tradeDate <= highTradeDateStr
		List<NotionHot> notionHotList = notionHotService
				.queryNotionHotByDateStrBetween(lowTradeDateStr, highTradeDateStr);
		...
		for (NotionHot notionHot : notionHotList) {
			...
			if (map.containsKey(notionName)) {
				map.put(notionName,
						map.get(notionName) + notionHot.getChangePercent());
			} else {
		...
		// 大盘指数总涨跌幅
		Map<String, Float> indexMap = indexService.queryTotalChangePctBetweenMap(
				lowTradeDateStr, highTradeDateStr);
		if (indexMap == null) {
		...
		// 概念板块的公司家数.
		Map<String, Integer> notionCorpsNumMap = notionInfoService
				.queryNotionCorpsNumMap(StockConstant.THS_FLAG);
		...
		for (Map.Entry<String, Float> entry : sortList) {
			...
			sb.append(CSV_SPLIT)
					...
					.append(CSV_SPLIT)
					.append(
							notionCorpsNumMap.containsKey(notionName) ? notionCorpsNumMap
									.get(notionName) : 0).append("\n");
		}
		sb.append("\n");
	...
	public StringBuilder getIndustryHotPhraseString(String lowTradeDateStr,
			String highTradeDateStr, boolean header) {
		long begin = System.currentTimeMillis();
		// lowTradeDateStr <= tradeDate <= highTradeDateStr
		List<IndustryHot> industryHotList = industryHotService
				.queryIndustryHotByDateStrBetween(lowTradeDateStr, highTradeDateStr);
		...
		for (IndustryHot industryHot : industryHotList) {
			...
			if (map.containsKey(industryName)) {
				map.put(industryName,
						map.get(industryName) + industryHot.getChangePercent());
			} else {
		...
		// 行业板块的公司家数.
		Map<String, Integer> industryCorpsNumMap = industryInfoService
				.queryIndustryCorpsNumMap(StockConstant.THS_FLAG);
		...
		for (Map.Entry<String, Float> entry : sortList) {
			...
			sb.append(CSV_SPLIT)
					...
					.append(CSV_SPLIT)
					.append(
							industryCorpsNumMap.containsKey(industryName) ? industryCorpsNumMap
									.get(industryName) : 0).append("\n");
		}
		sb.append("\n");
	...
	public StringBuilder getIndustryHotStocksPhraseString(
			String lowTradeDateStr, String highTradeDateStr, boolean header) {
		long begin = System.currentTimeMillis();
		// lowTradeDateStr <= tradeDate <= highTradeDateStr
		List<IndustryHotStocks> industryHotStocksList = industryHotStocksService
				.queryIndustryHotStocksByDateStrBetween(lowTradeDateStr,
						highTradeDateStr);
		...
		// 股票所属板块
		Map<String, String> stockIndustriesMap = industryHotStocksService
				.queryStocksIndustriesBetweenMap(lowTradeDateStr, highTradeDateStr);

		// 股票代码
		Map<String, String> codeMap = exchangeInfoService.queryStockNameCodeMap();

		// 股票区间总的涨跌幅, 这里仍然使用notion
		Map<String, Float> stockChangePctMap = notionHotStocksService
				.queryStocksChangePctBetweenMap(lowTradeDateStr, highTradeDateStr);
		...
		for (Map.Entry<String, Float> entry : sortList) {
			...
			String stockCode = codeMap.containsKey(stockName) ? codeMap
					.get(stockName) : "";
			String industryNameStr = stockIndustriesMap.containsKey(stockName) ? stockIndustriesMap
					.get(stockName) : "";

			sb.append(CSV_SPLIT).append(index).append(CSV_SPLIT).append(stockCode)
					.append(CSV_SPLIT).append(stockName).append(CSV_SPLIT)
	...
	public StringBuilder getNotionHotStocksPhraseString(String lowTradeDateStr,
		...
		long begin = System.currentTimeMillis();
		// lowTradeDateStr <= tradeDate <= highTradeDateStr
		List<NotionHotStocks> notionHotStocksList = notionHotStocksService
				.queryNotionHotStocksByDateStrBetween(lowTradeDateStr,
						highTradeDateStr);
		...
		// 股票所属板块
		Map<String, String> stockNotionsMap = notionHotStocksService
				.queryStocksNotionsBetweenMap(lowTradeDateStr, highTradeDateStr);
		...
		for (Map.Entry<String, Float> entry : sortList) {
			...
			String stockCode = codeMap.containsKey(stockName) ? codeMap
					.get(stockName) : "";
			String notionNameStr = stockNotionsMap.containsKey(stockName) ? stockNotionsMap
					.get(stockName) : "";
