 ...
 * @author leslie  
 */
public class PersistLHBInfo {
	private static Logger LOGGER = Logger.getLogger(PersistLHBInfo.class
			.getName());
	...
	public void persistLHB() {
		...
		File savedDir = new File(StockUtils.getDailyStockSaveDir(jobType));
		 ...
		 * 解析概念板块、行业板块内的股票信息.
		 */
		stockParseToolTHS.persistNotionIndustryHotStocksFromHtml(dateStr,
				"NOTION");
		stockParseToolTHS.persistNotionIndustryHotStocksFromHtml(dateStr,
				"INDUSTRY");
		 ...
		 */
		genFiles();
		LOGGER.info("JOB " + jobType + " genFiles finished. total time: "
				+ (System.currentTimeMillis() - begin) / (1000 * 60) + " min.");
	}
}
